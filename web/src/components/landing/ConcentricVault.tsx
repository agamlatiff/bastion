...
type SimulationType = 'double_click' | 'negative_balance' | 'tamper_log';

interface SimulationLog {
    time: string;
    text: string;
    status: 'info' | 'blocked' | 'success';
}
    ...
    const runSimulation = (type: SimulationType) => {
        ...
        if (type === 'double_click') {
            ...
            const steps: SimulationLog[] = [
                { time: '00:00.012', text: 'Kasir menekan tombol bayar 5x berturut-turut dalam 80 milidetik...', status: 'info' },
                { time: '00:00.024', text: 'Transaksi #1 (Rp 175.000) diterima & diproses...', status: 'success' },
                { time: '00:00.036', text: 'Transaksi #2 dicegat: Kunci transaksi ganda aktif (Dibatalkan otomatis)', status: 'blocked' },
                { time: '00:00.045', text: 'Transaksi #3 dicegat: Kunci transaksi ganda aktif (Dibatalkan otomatis)', status: 'blocked' },
                { time: '00:00.054', text: 'Transaksi #4 dicegat: Kunci transaksi ganda aktif (Dibatalkan otomatis)', status: 'blocked' },
                { time: '00:00.062', text: 'Transaksi #5 dicegat: Kunci transaksi ganda aktif (Dibatalkan otomatis)', status: 'blocked' },
            ];
            ...
            steps.forEach((step, idx) => {
                setTimeout(() => {
                    ...
                    if (idx === steps.length - 1) {
                        ...
                        setSimSummary({
                            title: 'Pencegahan Pembayaran Ganda Sukses',
                            result: '1 Transaksi Sah • 4 Percobaan Ganda Ditahan Otomatis',
                            guarantee: 'Saldo Pelanggan & Kas Toko Aman Rp 0 Selisih',
                            accentColor: '#38bdf8',
                        });
                    }
                }, (idx + 1) * 220);
            });
        } else if (type === 'negative_balance') {
            setActiveLayer(2);
            const steps: SimulationLog[] = [
                { time: '00:00.010', text: 'Permintaan pengeluaran kas: Rp 5.000.000...', status: 'info' },
                { time: '00:00.028', text: 'Mengecek saldo kas riil: Tersedia Rp 3.200.000', status: 'info' },
                { time: '00:00.042', text: 'Peringatan defisit kas: Kekurangan dana Rp 1.800.000 terdeteksi', status: 'blocked' },
                { time: '00:00.058', text: 'Pengeluaran kas otomatis dibatalkan sebelum uang berpindah tangan', status: 'blocked' },
            ];
            ...
            steps.forEach((step, idx) => {
                setTimeout(() => {
                    ...
                    if (idx === steps.length - 1) {
                        setIsSimulating(false);
                        setSimSummary({
                            title: 'Pencegahan Saldo Minus Sukses',
                            result: 'Pengeluaran Dibatalkan • Sisa Kas Tetap Utuh Rp 3.200.000',
                            guarantee: 'Kas Usaha Anda Tidak Pernah Jebol atau Berutang',
                            accentColor: '#10b981',
                        });
                    }
                }, (idx + 1) * 250);
            });
        } else if (type === 'tamper_log') {
            setActiveLayer(3);
            const steps: SimulationLog[] = [
                { time: '00:00.015', text: 'Percobaan revisi riwayat transaksi: Nota #TX-9042...', status: 'info' },
                { time: '00:00.035', text: 'Mengecek segel catatan buku kas...', status: 'info' },
                { time: '00:00.050', text: 'Akses penulisan ditolak: Riwayat transaksi bersifat permanen', status: 'blocked' },
                { time: '00:00.065', text: 'Integritas pembukuan terverifikasi: Data asli tetap dipertahankan', status: 'success' },
            ];
            ...
            steps.forEach((step, idx) => {
                setTimeout(() => {
                    ...
                    if (idx === steps.length - 1) {
                        setIsSimulating(false);
                        setSimSummary({
                            title: 'Perlindungan Catatan Kas Sukses',
                            result: 'Perubahan Ditolak • Catatan Terkunci Rapat Tanpa Celah',
                            guarantee: 'Pembukuan Jujur & Tidak Bisa Diubah Diam-Diam',
                            accentColor: '#818cf8',
                        });
                    }
                }, (idx + 1) * 250);
    ...
    return (
        <div className="w-full max-w-7xl mx-auto space-y-6">
            ...
            <div className="relative rounded-[2.5rem] border border-white/10 bg-[#09090e] p-5 sm:p-8 lg:p-12 overflow-hidden shadow-[0_30px_100px_rgba(0,0,0,0.9)]">
                ...
                {/* ========================================================================= */}
                {/* INTERACTIVE PROTECTION SIMULATOR (LABORATORIUM UJI KAS LANGSUNG)          */}
                {/* ========================================================================= */}
                <div className="relative z-10 pt-8 sm:pt-10 mt-8 border-t border-zinc-800/60 text-left">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                        <div className="space-y-1">
                            <div className="flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-sky-400 animate-ping" />
                                <h4 className="text-sm sm:text-base font-bold text-white tracking-tight">
                                    Uji Coba Proteksi Langsung (Simulator Kas)
                                </h4>
                            </div>
                            <p className="text-xs text-zinc-400">
                                Pilih salah satu skenario kendala di bawah untuk melihat bagaimana sistem Bastion seketika mencegatnya secara otomatis.
                            </p>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 shrink-0">
                            ...
                            >
                                ...
                                <span>Uji Klik Dobel 5x</span>
                            </button>

                            <button
                                type="button"
                                disabled={isSimulating}
                                onClick={() => runSimulation('negative_balance')}
                                className={`px-3 py-1.5 rounded-xl border text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer select-none ${
                                    activeSim === 'negative_balance'
                                        ? 'bg-emerald-500/20 border-emerald-400 text-emerald-300 shadow-sm'
                                        : 'bg-zinc-900/90 border-zinc-800 text-zinc-300 hover:border-zinc-700 hover:text-white'
                                }`}
                            >
                                <Lock className="w-3.5 h-3.5 text-emerald-400" />
                                <span>Uji Tarik Kas Minus</span>
                            </button>

                            <button
                                type="button"
                                disabled={isSimulating}
                                onClick={() => runSimulation('tamper_log')}
                                className={`px-3 py-1.5 rounded-xl border text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer select-none ${
                                    activeSim === 'tamper_log'
                                        ? 'bg-indigo-500/20 border-indigo-400 text-indigo-300 shadow-sm'
                                        : 'bg-zinc-900/90 border-zinc-800 text-zinc-300 hover:border-zinc-700 hover:text-white'
                                }`}
                            >
                                <Fingerprint className="w-3.5 h-3.5 text-indigo-400" />
                                <span>Uji Ubah Nota Kemarin</span>
                            </button>
                        </div>
                    </div>
                    ...
                    {activeSim && (
                        <div className="rounded-2xl border border-zinc-800 bg-[#050508] p-4 sm:p-5 font-mono text-xs space-y-3 shadow-2xl animate-in fade-in duration-200">
                            {/* Terminal Header */}
                            <div className="flex items-center justify-between border-b border-zinc-800/80 pb-2.5">
                                <div className="flex items-center gap-2 text-zinc-500 text-[11px]">
                                    ...
                                    <span className="ml-2 text-zinc-400">bastion-intercept-stream</span>
                                </div>
                                {isSimulating ? (
                            ...
                            <div className="space-y-1.5 py-1">
                                {simLogs.map((log, index) => (
                                    <div key={index} className="flex items-start gap-2.5 text-[11px] leading-relaxed">
                            ...
                            {simSummary && (
                                <div className="mt-3 pt-3 border-t border-zinc-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 rounded-xl bg-zinc-900/70 border border-zinc-800">
                                    <div className="space-y-0.5">
                                        <div className="flex items-center gap-1.5 text-xs font-bold text-white">
                                            <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                                            <span>{simSummary.title}</span>
                                        </div>
                                        <p className="text-[11px] text-zinc-400">{simSummary.result}</p>
                                    </div>
                                    <div className="sm:text-right shrink-0">
                                        <span className="text-[10px] text-zinc-500 block">Jaminan Kas:</span>
                                        <span className="text-xs font-bold font-mono text-emerald-400">
                                            {simSummary.guarantee}
                                        </span>
                                    </div>
                                </div>
                            )}
